  ...
  booknow() {
    ...
    if (!this.sortedEvent?.batches?.length) {
      ...
      this.router.navigate([
        '/user/trekregistration', this.event.eventName, this.event._id, this.selectedBatch?.fees, this.selectedBatch?.eventDate, this.selectedBatch?.batchId, this.event.pickupLocation.join(',')
      ]);
    }
  }
}